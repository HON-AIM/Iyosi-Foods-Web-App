...
export async function sendVerificationEmail(email: string, name: string, verificationLink: string) {
  ...
  await transporter.sendMail({
    from: `"Iyosi Foods Foods" <${process.env.EMAIL_FROM || "israelmiracle12@gmail.com"}>`,
    to: email,
    subject: "Verify your email address - Iyosi Foods Foods",
    text: `Welcome to Iyosi Foods Foods, ${safeName}!\n\nPlease verify your email by visiting: ${verificationLink}\n\nThis link expires in 24 hours.\n\nIf you didn't request this, you can safely ignore this email.`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 500px; margin: auto; padding: 20px; border: 1px solid #eaeaea; border-radius: 10px;">
        <h2 style="color: #333;">Welcome to Iyosi Foods Foods, ${safeName}!</h2>
...
export async function sendPasswordResetEmail(email: string, name: string, resetLink: string) {
  ...
  await transporter.sendMail({
    from: `"Iyosi Foods Foods" <${process.env.EMAIL_FROM || "israelmiracle12@gmail.com"}>`,
    to: email,
    subject: "Reset your password - Iyosi Foods Foods",
    text: `Reset Your Password\n\nHello ${safeName},\n\nWe received a request to reset the password for your Iyosi Foods Foods account.\n\nClick the link below to reset your password (expires in 1 hour):\n${resetLink}\n\nIf you didn't request a new password, you can safely ignore this email.`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 500px; margin: auto; padding: 20px; border: 1px solid #eaeaea; border-radius: 10px;">
        <h2 style="color: #333; text-align: center;">Reset Your Password</h2>
        <p style="color: #555; font-size: 16px;">Hello ${safeName},</p>
        <p style="color: #555; font-size: 16px;">We received a request to reset the password for your Iyosi Foods Foods account associated with this email address.</p>
...
export async function sendAdminDirectMessage(
  ...
  await transporter.sendMail({
    from: `"Iyosi Foods Store" <${process.env.EMAIL_FROM || "israelmiracle12@gmail.com"}>`,
    to: email,
    subject: safeSubject,
...
export async function sendOrderStatusUpdate(
  ...
  await transporter.sendMail({
    from: `"Iyosi Foods Store" <${process.env.EMAIL_FROM || "israelmiracle12@gmail.com"}>`,
    to: email,
    subject: `Order Update: ${safeStatus} - Iyosi Foods Foods`,
    text: `Hello ${safeName},\n\nYour order #${safeOrderId} has been updated to: ${safeStatus}.\n\nThank you for shopping with Iyosi Foods Foods.`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 500px; margin: auto; padding: 20px; border: 1px solid #eaeaea; border-radius: 10px;">
        ...
        <p style="color: #888; font-size: 12px; text-align: center;">Thank you for shopping with Iyosi Foods Foods.</p>
      </div>
    `,
  });
}
...
export async function sendOrderConfirmationEmail(
  ...
  await transporter.sendMail({
    from: `"Iyosi Foods Foods" <${process.env.EMAIL_FROM}>`,
    to: email,
    subject: `Order Confirmed — ${order.orderNumber} | Iyosi Foods Foods`,
    ...
    html: `
      ...
      <body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;">
        ...
        <p style="font-size:12px;color:#9ca3af;text-align:center;">
          ...
          © Iyosi Foods Foods. All rights reserved.
        </p>
      </body>
      </html>
    `,
