  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.status === 401 || response.status === 403) {
        toast.error("Unauthorized: You do not have permission to update products");
        router.push("/login");
        return;
      }
      ...
      if (response.ok) {
        ...
        toast.error(data.message || "Failed to update product. Please try again.");
      }
    } catch (error) {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="flex flex-col items-center space-y-3">
          ...
          <p className="text-gray-500">{isCheckingAuth ? "Checking authorization..." : "Loading product..."}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <Link href="/admin/products" className="mt-2 text-sm font-medium text-red-600 hover:text-red-700 inline-block">
              Back to Products
            </Link>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              aria-describedby={formErrors.description ? "description-error" : "description-count"}
            ...
            <div className="flex items-center gap-4">
              <div className="relative h-24 w-24 rounded-lg overflow-hidden border-2 border-gray-200 bg-gray-50 flex-shrink-0">
                {imagePreview ? (
                  ...
                  <div className="h-full w-full flex items-center justify-center">
                    <PackageX className="h-8 w-8 text-gray-400" aria-hidden="true" />
                  </div>
                )}
              </div>
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-500 mt-2">
                  JPEG, PNG, or WebP &bull; Max 5MB
                  {formData.image && !imageFile && " &bull; Current image will be kept"}
                </p>
              </div>
            </div>
          </div>
